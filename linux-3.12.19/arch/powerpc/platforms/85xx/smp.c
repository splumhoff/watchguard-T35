...
extern int qman_warm_reset(void);
#endif
#ifdef CONFIG_FSL_BMAN_CONFIG
extern int bman_warm_reset(void);
#endif
#ifdef CONFIG_KEXEC
#ifdef CONFIG_PPC32
...
void mpc85xx_smp_kexec_cpu_down(int crash_shutdown, int secondary)
{
	...
	if (cpu == crashing_cpu && cpu_thread_in_core(cpu) != 0) {
		/*
		 * We enter the crash kernel on whataver cpu crashed,
		 * even if it's a secondary thread.  If that's the case,
		 * disable the corresponding primray thread.
		 */
	...
	if (disable_threadbit) {
		while (paca[disable_cpu].kexec_state < KEXEC_STATE_REAL_MODE) {
			...
			if (!notified && now - start > 1000000) {
				pr_info("%s: waiting for cpu %d (physical %d) to enter KEXEC_STATE_REAL_MODE\n",
					__func__, disable_cpu,
					paca[disable_cpu].hw_cpu_id);
				notified = true;
			}
		}
