...
static inline int is_mfsc_port(struct phy_device *phydev)
{
  ...
  return ((wg_cpu_model == WG_CPU_T2081 || wg_cpu_model == WG_CPU_T1042) && 
		  (phydev->addr == 0 || phydev->addr == 1 || phydev->addr == 2));
}
...
static void wg_handle_fixed_partner(struct phy_device *phydev)
{
	...
	if(phydev->addr != 0 && phydev->addr != 1 && phydev->addr != 2) {
...
void phy_state_machine(struct work_struct *work)
{
	...
	switch(phydev->state) {
		...
		case PHY_RUNNING:
			...
			if (memcmp(phydev->attached_dev->name, "sw", 2) != 0)
#endif
				phydev->state = PHY_CHANGELINK;
			break;
		case PHY_CHANGELINK:
