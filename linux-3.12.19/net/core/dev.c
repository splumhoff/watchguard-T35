...
extern	int		wg_backlog_high; // Backlog queue high    water mark
extern	atomic_long_t	wg_drop_backlog; // Drop due to backlog queue full
extern	atomic_long_t	wg_drop_length;	 // Drop due to length error
extern	atomic_long_t	wg_drop_unknown; // Drop due to unknown type
...
#ifdef CONFIG_WG_PLATFORM
		atomic_long_inc(&wg_drop_length);
#else
		atomic_long_inc(&dev->rx_dropped);
#endif
		kfree_skb(skb);
...
#ifdef	CONFIG_WG_PLATFORM // WG:JB HW bridging
	if (unlikely(wg_br_bridged & dev->wg_if_map)) {
		if (likely(wg_br_primary & dev->wg_if_map))
			goto wg_bridge;
		kfree_skb(skb);
		rc = NET_XMIT_DROP;
		goto out;
...
#ifdef CONFIG_WG_PLATFORM
	if (unlikely(qlen > wg_backlog_high))
		wg_backlog_high = skb_queue_len(&sd->input_pkt_queue);
#endif
...
#ifdef	CONFIG_WG_PLATFORM
	atomic_long_inc(&wg_drop_backlog);
#else
...
drop:
#ifdef CONFIG_WG_PLATFORM
		atomic_long_inc(&wg_drop_unknown);
#else
		atomic_long_inc(&skb->dev->rx_dropped);
#endif
		kfree_skb(skb);
...
#ifdef	CONFIG_WG_PLATFORM_QUEUE // WG:JB wg_queue hooks

#ifdef	CONFIG_X86	 // WG:JB unlikely on x86
#define	wg_que_on	unlikely
#endif

#ifdef	CONFIG_PPC	 // WG:JB   likely on ppc
#define	wg_que_on	likely
#endif

// Pointer to PSS or DSA packet receive
int		(*wg_que_recv)(struct sk_buff*);
EXPORT_SYMBOL_GPL(wg_que_recv);

// Pointer to NetQ polling function
int	    (*wg_NetQ_poll)(void);
EXPORT_SYMBOL(wg_NetQ_poll);

// Pointer to NetQ add     function
int	    (*wg_NetQ_add)(struct sk_buff*, int);
EXPORT_SYMBOL(wg_NetQ_add);
...
#ifdef	CONFIG_WG_PLATFORM_QUEUE // WG:JB wg_queue hooks
int wg_netif_receive_skb(struct sk_buff *skb)
{
	return __netif_receive_skb(skb);
}
EXPORT_SYMBOL(wg_netif_receive_skb);
#endif
...
#ifdef	CONFIG_WG_PLATFORM_QUEUE // WG:JB wg_queue hooks
	{
		register int q = -ENOSYS;

		if (wg_que_on(wg_que_recv)) {
			q = wg_que_recv(skb);
			if (unlikely(q == -ENODATA))
				return NET_RX_DROP;
		}

		if (wg_que_on(q != -ENOSYS))
		if (wg_que_on(wg_NetQ_add))
		{
			if (q < 0) q = raw_smp_processor_id();
			q = wg_NetQ_add(skb, q);
			if (q >= 0) return q;
		}
	}
...
#ifdef	CONFIG_WG_PLATFORM_QUEUE // WG:JB wg_queue hooks
	if (wg_que_on(wg_NetQ_poll)) wg_NetQ_poll();
#endif
...
#ifdef	CONFIG_WG_PLATFORM // WG:JB Set up bitmap of iflink
	dev->wg_if_map = WG_BR_MAP(dev->iflink);
	if (unlikely(dev->ifindex != dev->iflink))
		printk(KERN_DEBUG "%s: ifindex %2d -> iflink %2d %s\n",
		       __FUNCTION__, dev->ifindex, dev->iflink, dev->name);
#endif
...
#ifdef	CONFIG_WG_PLATFORM // WG:JB Decrement stuck  ref count
			if (refcnt > 0) dev_put(dev);
#endif
			pr_emerg("unregister_netdevice: waiting for %s to become free. Usage count = %d\n",
				 dev->name, refcnt);
			warning_time = jiffies;
#ifdef	CONFIG_WG_PLATFORM // WG:JB Exit on negative ref count
