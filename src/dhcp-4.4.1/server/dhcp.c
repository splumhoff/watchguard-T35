		...
		const char *file, int line)
{
	...
	if (oc &&
		...
		if (find_hosts_by_uid (&hp, client_identifier.data,
			...
			packet -> known = 1;
			find_static_lease (&fixed_lease, packet, share, hp);
		}
		...
		find_lease_by_uid (&uid_lease, client_identifier.data,
				   client_identifier.len, MDL);
		if (uid_lease && uid_lease -> flags & STATIC_LEASE) {
	...
	if (!fixed_lease && !host) {
		if (find_hosts_by_haddr (&hp, packet -> raw -> htype,
			...
			host_reference (&host, hp, MDL);
			host_dereference (&hp, MDL);
			find_static_lease (&fixed_lease, packet, share, host);
#if defined (DEBUG_FIND_LEASE)
			if (fixed_lease) {
	...
	if (hw_lease && hw_lease -> flags & STATIC_LEASE) {
	...
	if (ip_lease && ip_lease -> flags & STATIC_LEASE) {
...
int find_static_lease (struct lease **lp, struct packet *packet,
		  struct shared_network *share, struct host_decl *hp)
{
	...
	if (lease_allocate (&lease, MDL) != ISC_R_SUCCESS)
		return 0;
	if (!find_host_for_network (&lease -> subnet, 1,
	...
	/* The static lease has been entered on startup with host declarations */
	if (!find_lease_by_ip_addr (lp, lease -> ip_addr, MDL)) {
		log_error ("Error: no static lease found for the fixed address %s",
			   piaddr (lease -> ip_addr));
		lease_dereference (&lease, MDL);
		host_dereference (&rhp, MDL);
		return 0;
	...
	if (!(lease -> flags & STATIC_LEASE)) {
		log_error ("Warning: a non-static lease is found for a fixed address %s",
			   piaddr (lease -> ip_addr));
		lease_dereference (&lease, MDL);
		host_dereference (&rhp, MDL);
		return 0;
		    ...
		    struct pool *pool, int *peer_has_leases)
{
	...
	for (; pool ; pool = pool -> next) {
...
#endif
		{
			...
			for (candl = LEASE_GET_FIRST(lc);
			     candl;
			     candl = LEASE_GET_NEXT(lc, candl)) {
				if (!(candl->flags & STATIC_LEASE))
					break;
			}
		}
