...
static int enter_static_lease (struct host_decl *hp)
{
	...
	for (share = shared_networks; share; share = share -> next) {
		if (find_host_for_network (&lease -> subnet, 0,
	...
	/* If an existing static lease is found, give a warning. */
	if (find_lease_by_ip_addr (&lp, lease -> ip_addr, MDL)) {
		log_error ("Warning: an existing lease is found for the fixed address %s",
	...
	/* Simply reference the static lease to the first pool of the shared network */
	pool_reference (&lease -> pool, share -> pools, MDL);

	/* Remember static lease in the IP address hash. */
...
int find_host_for_network (struct subnet **sp, int all, struct host_decl **host,
	...
	struct lease *lease;
{
	...
	if (find_lease_by_ip_addr (&comp, lease -> ip_addr, MDL)) {
		/* In case a static lease is found, drop it */
		if (comp -> flags & STATIC_LEASE) {
			log_error ("Warning: static lease found, dropping lease for %s",
				   piaddr (lease -> ip_addr));
