...
setupCafConfig() {
	...
	if [ -n "$value" ]; then
		if [ -d "$rconfigDir" ]; then
			for file in $(egrep -l "$pattern" $rconfigDir/*); do
		...
		if [ -d "$rscriptDir" ]; then
			for file in $(egrep -l "$pattern" $rscriptDir/*); do
				basefile=$(basename $file)
				#echo "Replacing $pattern with $value - $basefile"
				sed -i "s?$pattern?$value?g" $file
			done
...
ln -sf librabbitmq.so.4.0.1 librabbitmq.so
ln -sf librabbitmq.so.4.0.1 librabbitmq.so.4

#Run provider install logic
installPProviders="$scriptDir"/installPythonProviders.sh
